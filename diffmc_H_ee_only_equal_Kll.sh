...
for h in "${H_ARRAY[@]}"
   do
     ...
     sed -e 's/'"$OLD"'/'"$NEW"'/g' optics.cpp.template.H.ee_only.equal_Kll > optics.cpp
     make
...
#     WORK_DIR=output-$(hostname)-ee_only_equal_Kll-${h}T-0.01-e-pi_2-$(date +%Y%m%d)

#     OEPROFILE_FILE="oeprofile_${h}T_0.01_starkexp_3.txt"
#     EOPROFILE_FILE="eoprofile_${h}T_0.01_starkexp_3.txt"
#     EEPROFILE_FILE="eeprofile_${h}T_0.01_starkexp_equal_Kll.txt"
#     OFREEPATH_FILE="ofreepath_${h}T_starkexp_3.txt"
#     EFREEPATH_FILE="efreepath_${h}T_starkexp_3_ee_only_equal_Kll.txt"
#     ECHANNELPROB_FILE="echannel_ee_only.txt"

     WORK_DIR=output-$(hostname)-ee_only_equal_Kll_mean_freepath-${h}T-0.01-e-pi_2-$(date +%Y%m%d)

     OEPROFILE_FILE="oeprofile_${h}T_0.01_starkexp_3.txt"
     EOPROFILE_FILE="eoprofile_${h}T_0.01_starkexp_3.txt"
     EEPROFILE_FILE="eeprofile_${h}T_0.01_starkexp_equal_Kll.txt"
     OFREEPATH_FILE="ofreepath_${h}T_starkexp_3.txt"
     EFREEPATH_FILE="mean_efreepath_${h}T_starkexp_3_ee_only_equal_Kll.txt"
     ECHANNELPROB_FILE="echannel_ee_only.txt"
