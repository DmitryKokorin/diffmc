...
for h in "${H_ARRAY[@]}"
   do
     ...
     sed -e 's/'"$OLD"'/'"$NEW"'/g' optics.cpp.template.H.ee_only > optics.cpp
     make

     WORK_DIR=output-$(hostname)-ee_only-starkexp-${h}T-0.01-e-pi_2-$(date +%Y%m%d)
     ...
     EFREEPATH_FILE="efreepath_${h}T_starkexp_3_ee_only.txt"
